...
var mainApp1 = angular.module("alliance", ['ngRoute', 'ngCart','720kb.socialshare']);

mainApp1.config(function($routeProvider, $locationProvider) {
	$routeProvider
	// route for the home page
	.when('/', {
		title :'Alliance Gifting | A corporate gifting company',
		templateUrl : 'views/home.html',
	}).when('/about-us', {
		title :'About Us',
		templateUrl : 'views/about.html'
	}).when('/contact-us', {
		title :'Contact Us | A corporate gifting company',
...
mainApp1.controller('all-controller',function($scope, $http){
	
})



mainApp1.run(['$rootScope', function($rootScope) {
...
mainApp1.directive('loading',   ['$http' ,function ($http)
{
	return {
		restrict: 'A',
...
mainApp1.directive("loader", function ($rootScope) {
...
mainApp1.controller('thumb-ctrl', function($scope, $http) {
	$scope.selectedCountryId = "";
	$scope.showLoader = true;
	$http({
		method : "GET",
		url : "http://localhost/Alliance/admin/index.php/book/cards",
	}).then(function(response) {
		$scope.cards = response.data.cards;
		$scope.average = response.data.cards;
		$scope.average_rating = response.data.cards.average_rating;	
		$scope.showLoader = true;
		starts_update();
	});
	...
	// $http({
	// 	method : "GET",
	// 	url : "http://localhost/Alliance/admin/index.php/book/prices",
	// }).then(function(response) {
	// 	$scope.prices_id = response.data;
	// });
	...
	$scope.priceFilter = function(cards) {
		if ($scope.priceIncludes.length > 0) {
			if ((parseInt(cards.price) >= parseInt($scope.ranges[0])) && (parseInt(cards.price) <= parseInt($scope.ranges[1])))
				return cards;
		} else {
			return cards;
		}
...
mainApp1.controller('view-cards', ['$scope', '$http', '$sce','$routeParams','$rootScope', function($scope, $http,$sce,$routeParams,$rootScope) {	
	id = $routeParams.id;
	$scope.showLoader = true;
	$scope.fn1 = function() {
		$http({
			method : "GET",
			url : "http://localhost/Alliance/admin/index.php/book/card/"+id+"",
		}).then(function(response) {
			$scope.showLoader = true;
			$scope.cards = response.data.card;
			$scope.rating = response.data.ratings;
			if(response.data.comments != 0){
				$scope.comment = response.data.comments;
			}
			$scope.relatedcards = response.data.related_cards;
			$scope.topcards = response.data.top_card;
			$rootScope.title = $scope.cards.title+' | gift Cards';      
...
mainApp1.controller('view-cards-sub',function($scope, $http) {
	$scope.submitreply = function(parentid,cardid) {
		$scope.parentId =parentid;
		$scope.cardId   = cardid;
		$http({
			method: 'POST',
			url:  'http://localhost/Alliance/admin/index.php/book/reply',
			data: {				
				'card_id': $scope.cardId,
				'parent_comment_id': $scope.parentId,
				'comment':$scope.commenting,
				'comment_sender_name':$scope.comment_sender_name
			}
...
mainApp1.controller('view-cards-sub-comment',function($scope, $http) {
	$scope.submitcomment = function(cardid) {
		$scope.cardId   = cardid;
		$http({
			method: 'POST',
			url:  'http://localhost/Alliance/admin/index.php/book/comment',
			data: {				
				'card_id': $scope.cardId,
				'parent_comment_id': '0',
				'comment':$scope.commenting,
...
mainApp1.controller('cart-id-testing', function($scope, $http) {
...
mainApp1.controller('cart-test', function($scope, $http) {
	$scope.cart = function() {
		$http({
			method : "POST",
			url : "http://localhost/Alliance/gateway/pay.php",
			data : {
...
mainApp1.controller('myCtrl', ['$scope', '$http', 'ngCart',
	function($scope, $http, ngCart) {
		$scope.Productname = ngCart.getCart().items;
		$scope.total = ngCart.totalCost();
		angular.forEach($scope.Productname, function(value, key){
...
mainApp1.controller("employeecontroller", ['$scope', '$http', 'ngCart','$route',
	function($scope, $http, ngCart,$route) {
		$scope.insertdata = function() {
			$http({
				method : "POST",
				url : "http://localhost/Alliance/admin/index.php/book/contact_form",
				data : {
...
function alliance_success() {
	$('#contact_success').show()	
}

mainApp1.controller('userCtrl', ['$scope', '$http',
	function($scope, $http) {
		$scope.upload = function() {
			...
			fd.append('title', $scope.texts);
			console.log(fd)
			$http({
				method : 'post',
				url : 'http://localhost/Alliance/admin/index.php/book/do_uploads',
				data : fd,
				headers : {
...
mainApp1.controller('thumb', function($scope) {
	$scope.getname = function(data) {
		$scope.name = data;
	}
});

mainApp1.controller('adminctrl', ['$scope', '$http', '$cookies',
	function($scope, $http, $window, $cookies) {
		$scope.login = function() {
			$http({
				method : 'POST',
				url : 'http://localhost/Alliance/admin/index.php/auth/login',
				data : {
...
mainApp1.controller('StarCtrl', ['$scope','$http','$window', function ($scope,$http,$window) {
	$scope.rating = 0;
	$scope.ratings = [{
		current: 1,
		max: 5
	...
	$scope.sendRate = function(card_id){
		$scope.cardid = card_id;
		console.log($scope.cardid,$scope.ratings[0].current);
		$http({
			method: 'POST',
			url:  'http://localhost/Alliance/admin/index.php/book/review',
			data: {
				'card_id': $scope.cardid,
				'rating':$scope.ratings[0].current,
				'name':$scope.review_name,
				'email':$scope.review_email,
				'phone':$scope.review_phone
			}
          }).then(function (response) {
          	$window.location.reload();
          	alliance_success_feedback();
          }, function (response) {
          });
...
function alliance_success_feedback() {
	$('#feedback_success').show()	
}

mainApp1.directive('starRating', function () {
	return {
		restrict: 'A',
...
mainApp1.filter('format', function () {
...
mainApp1.filter("trust", ['$sce', function($sce) {
	return function(htmlCode){
		return $sce.trustAsHtml(htmlCode);
	}
}]);

mainApp1.filter('spaceless',function() {
...
$(document).on("click", ".nav-togglebar,.mobile-header .fa-times, .body-overlay", function(e) {
	e.preventDefault();
	if ($('#navigation-collapse').hasClass("open")) {
		$('#navigation-collapse').removeClass("open");
		$('html, body').css('overflowY', 'auto');
		$('.body-overlay').css('opacity', '0').css('z-index', '-1');
	} else {
		$('#navigation-collapse').addClass("open");
		$('html, body').css('overflowY', 'hidden');
		$('.body-overlay').css('opacity', '1').css('z-index', '9');
	}
}); 
